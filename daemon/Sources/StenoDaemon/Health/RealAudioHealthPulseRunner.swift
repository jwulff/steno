...
public struct RealAudioHealthPulseRunner: HealthPulseRunning {
    ...
    private let requiredSources: @Sendable () async -> Set<AudioSourceType>
    ...
    public init(
        ...
        requiredSources: @Sendable @escaping () async -> Set<AudioSourceType> = { [.microphone] },
    ...
    public func run(trigger: HealthPulseTrigger) async -> HealthPulseRunResult {
        ...
        let started = now()
        let sources = await requiredSources()
        let required = sources.isEmpty ? Set<AudioSourceType>([.microphone]) : sources
        ...
        var bestText = ""
        var bestScore = 0.0
        var bestBySource: [AudioSourceType: (text: String, score: Double)] = [:]
        ...
        while now() <= deadline {
            ...
            for source in required {
                let sourceObserved = segments
                    .filter { $0.source == source }
                    .map(\.text)
                    .joined(separator: " ")
                let sourceScore = Self.similarity(expected: phrase, observed: sourceObserved)
                if sourceScore > (bestBySource[source]?.score ?? 0) {
                    bestBySource[source] = (sourceObserved, sourceScore)
                }
            }

            let missing = required.filter { (bestBySource[$0]?.score ?? 0) < threshold }
            if missing.isEmpty {
                return HealthPulseRunResult(
                    state: .passed,
                    expectedText: phrase,
        ...
        let missing = required
            .filter { (bestBySource[$0]?.score ?? 0) < threshold }
            .map(\.rawValue)
            .sorted()
            .joined(separator: ", ")
        ...
        return HealthPulseRunResult(
            ...
            threshold: threshold,
            message: "Health pulse did not observe the expected token on \(missing) within \(Int(timeoutSeconds))s"
        )
    }
