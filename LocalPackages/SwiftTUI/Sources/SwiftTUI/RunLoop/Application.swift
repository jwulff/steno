...
public class Application {
    ...
    /// Global key handlers that are checked before passing events to the first responder.
    /// Keys in this dictionary will trigger their associated callbacks regardless of focus.
    /// Note: This runs on the main thread, so access is inherently serialized.
    public nonisolated(unsafe) static var globalKeyHandlers: [Character: () -> Void] = [:]
    ...
    private func handleInput() {
        ...
        for char in string {
            if arrowKeyParser.parse(character: char) {
                ...
                stop()
            } else if let handler = Application.globalKeyHandlers[char] {
                // Global key handler takes precedence over first responder
                handler()
            } else {
